...
# hints
# - number: Use numeric hints. (In this mode you can also type letters from the
# hinted element to filter and reduce the number of elements that are hinted.)
# letter: Use the characters in the `hints.chars` setting.  - word: Use hints
# words based on the html elements and the extra words.
c.hints.mode = 'letter'
...
# either the hint string (word mode) or filter (number mode).  - full-match:
# Follow the hint when the user typed the whole hint (letter, word or number
# mode) or the element's text (only in number mode).  - never: The user will
